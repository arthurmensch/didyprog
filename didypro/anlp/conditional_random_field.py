...
class ConditionalRandomField(torch.nn.Module):
    ...
    def _input_likelihood(self, logits: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        potentials = (logits[:, :, :, None]
                      + self.transitions[None, None, :, :])
        potentials[:, 0, :, :] = logits[:, 0, :, None].expand(-1, -1, num_tags)

        if self.include_start_end_transitions:
            potentials[:, 0, :, :] += self.start_transitions[None, :, None]
            for i, end in enumerate(lengths):
                # Maybe unefficient
                potentials[i, end - 1, :, :] += self.end_transitions[None, :]

        packed_potentials = pack_padded_sequence(potentials,
                                                 lengths=lengths,
                                                 batch_first=True)

        return self.viterbi(packed_potentials)
